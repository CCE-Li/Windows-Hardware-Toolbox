...
- **进程页**: NtQuerySystemInformation 一次性获取进程表 (PID/PPID/CPU 时间/线程/句柄/会话),
  ...
  CPU% 通过两次采样差值归一化 (总和约等于系统总占用)。支持:
  - 进程图标 (SHGetFileInfo 提取, D3D11 纹理缓存, 渐进加载)
  - 搜索过滤、点击进程自动显示详细信息 (命令行/可执行路径/用户名)
  - 树形视图 (子进程归并到父进程, 可展开/折叠)
  - 排序、右键菜单 (结束任务/结束进程树/暂停/恢复/重启/设置优先级/CPU 亲和性/复制信息)
...
- ACPI 热区温度 (WMI `MSAcpi_ThermalZoneTemperature`, 十分之一开尔文换算为摄氏度),
  配合 CPU 频率/使用率、GPU 使用率、内存使用率、NVMe 温度等系统遥测, 每项带历史曲线。
- 缺失的传感器 (风扇/电压/GPU 温度等) 显示 N/A, 不伪造数值; 计划以可选插件形式接入
  NVAPI / AMD ADL / Intel ML。
